...
export class AuthService {
  ...
  Caso faça a conexão:
    ...
    ** Then -> Retorna res.status === true caso usuário exista
  ...
  */
  async login(body): Promise<any> {
    if (this.allow.guardian([body.email, body.password])) {
      ...
      this.api
        .get(environment.api.controllers.user.login, params)
        .then((res) => {
          console.log(res);
          if (res.status) {
            this.userClass.set(res);
            this.cache.setCache(
              this.userClass.getCachePath(),
              this.userClass.get()
            );
            this.master.getAll();
            this.screen.presentToast(
              'Bem Vindo, ' + this.userClass.get().data.first_name + '!',
              'sucess'
            );
          } else {
